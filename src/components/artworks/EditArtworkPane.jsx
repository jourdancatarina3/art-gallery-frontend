...
function EditArtworkPane(params) {
    ...
    const updatedImages = prevArtwork? (newImages.length > 0 || JSON.stringify(prevArtwork.images.map(image => image.image_url)) !== JSON.stringify(imagesUrl)) : false;
        ...
        <main className='font-Adamina container mx-auto my-5 min-h-lvh'>
            ...
            <div className="flex flex-wrap justify-between items-center mb-5 px-3 gap-3">    
                <FontAwesomeIcon
                    onClick={() => {
                        if (hasNewChanges) {
                            const res = window.confirm('Are you sure you want to leave this page?');
                            if (res) {
                ...
                <button
                    disabled={isLoading || !hasNewChanges || isUploading || isCreatingCategory || isUploading}
                    onClick={handleSave}
                        ...
                        <div className='w-[50%]'>
                            ...
                            <label class="form-control w-full">
                                ...
                                <label className="input input-bordered flex items-center gap-2 rounded-sm">
                                    ₱
                                    <input onChange={(e) => setPrice(e.target.value)} value={price} type="text" className="grow" placeholder="0.00" />
                                </label>
                            </label>
                            )}
                        </div>
